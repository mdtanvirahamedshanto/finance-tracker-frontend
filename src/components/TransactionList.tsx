...
export const TransactionList = ({
  ...
  const getCategoryColor = (category: string) => {
    const colors: Record<string, string> = {
      ...
      Familly: "bg-green-100 text-green-800 border-green-200",
      Bills: "bg-red-100 text-red-800 border-red-200",
      Loan: "bg-red-100 text-red-800 border-red-200",
      "Side Hustle": "bg-red-100 text-red-800 border-red-200",
      Investment: "bg-red-100 text-red-800 border-red-200",
      Other: "bg-gray-100 text-gray-800 border-gray-200",
      Gift: "bg-pink-100 text-pink-800 border-pink-200",
  ...
  const handleAddTransaction = async (
    newTransaction: Omit<Transaction, "_id">
  ) => {
    try {
  ...
  const handleEditTransaction = async (updatedTransaction: Transaction) => {
    try {
      await transactionAPI.update(updatedTransaction._id, updatedTransaction);
      // Refresh transactions
      const data = await transactionAPI.getAll();
      setTransactions(data);
      toast({
                ...
                <div className="flex items-start sm:items-center space-x-3 flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 mb-1">
                      ...
                      <Badge
                        variant="outline"
                        className={`text-xs self-start sm:self-auto ${getCategoryColor(
